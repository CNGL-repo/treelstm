...
Training script for sentiment classification on the SST dataset.
  ...
  -s,--seed   (default 0)            Seed for random numbers, 0 = obtain from system
  -o,--open   (default None)         Read model from file (and don't write model)
  -t,--test   (default test)         Name of test data folder (relative to data dir)
  -b,--binary                        Train and evaluate on binary sub-task
]]

if not (args.seed == 0) then
...
local test_dir = data_dir .. args.test .. '/'
local dependency = (args.model == 'dependency')
local train_dataset
local dev_dataset
if (args.open == 'None') then
  train_dataset = treelstm.read_sentiment_dataset(train_dir, vocab, fine_grained, dependency)
  collectgarbage()
  dev_dataset = treelstm.read_sentiment_dataset(dev_dir, vocab, fine_grained, dependency)
  collectgarbage()
  printf('num train = %d\n', train_dataset.size)
  printf('num dev   = %d\n', dev_dataset.size)
end
...
local best_dev_score
local best_dev_model
if not (args.open == 'None') then
  -- load a saved model
  best_dev_model = model_class.load(args.open)
else
  -- initialize model
...
-- attempt to index local 'best_dev_model' (a nil value)
...
if (args.open == 'None') and (lfs.attributes(treelstm.models_dir) == nil) then
...
while true do
  predictions_save_path = string.format(
    treelstm.predictions_dir .. '/sent-%s.%s.%dl.%dd.%d.%d.pred', args.model, subtask, args.layers, args.dim, file_idx, args.seed)
  model_save_path = string.format(
    treelstm.models_dir .. '/sent-%s.%s.%dl.%dd.%d.%d.th', args.model, subtask, args.layers, args.dim, file_idx, args.seed)
